...
function _apply_dir() {
    if test $# -ne 2
    then
        echo "Usage: _apply_dir [source dir] [destination dir]";
        exit -1;
    fi
...
function _apply_file() {
    if test $# -ne 2
    then
        echo "Usage: _apply_file [source file] [destination file]";
        exit -1;
    fi
...
function apply_bcache_module() {
    ...
    bcache_module_dst_dir="$1/$bcache_module_src_dir";
     ...
     _apply_dir $bcache_module_src_dir $bcache_module_dst_dir
}
...
function apply_bcache_api() {
   ...
   _apply_file $bcache_api_src_file $bcache_api_dst_file
}
